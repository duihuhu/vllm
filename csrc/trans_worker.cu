        ...
        if(!task_queue.empty()) {
            ...
            switch (task_type) {
                ...
                case TaskType::TRANSFER_SEND_LAYER_BLOCKS:
                    ...
                    trans_engine.send_comms_layer_blocks(task_meta.channel, task_meta.request_id, task.blocks, task.opposite_ranks[rank], task.layer, layer==(num_layer)-1, comms[use_comm], streams[use_comm], use_comm);
                    break;
                case TaskType::TRANSFER_RECV_LAYER_BLOCKS:
                    ...
                    for(int layer = 0 ;layer < num_layer; layer++) {
                        ...
                        trans_engine.recv_comms_layer_blocks(task_meta.channel, task_meta.request_id, task.blocks, task.opposite_ranks[rank], layer, layer==(num_layer)-1, [use_comm], streams[use_comm], use_comm);
