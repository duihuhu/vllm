...
if triton.__version__ >= "2.1.0":
    ...
    def _block_fwd_kernel(
        ...
        B_Loc, # block_table
        sm_scale,
        B_Start_Loc, #新token起始的地方
        B_Seqlen, #新token长度
        B_Ctxlen, #老token长度
        block_size,
        x,
        Out,
        stride_b_loc_b, #[num_seqs, max_blocks_per_seq]
        stride_b_loc_s,
        stride_qbs,
        stride_qh,
        stride_qd,
        ...
        for start_n in range(0, cur_batch_ctx_len, BLOCK_N):
            ...
            bn = tl.load(B_Loc + cur_batch * stride_b_loc_b +
                         ...
                         other=0)
            off_k = (bn[None, :] + layer_id * stride_k_cache_h * stride_k_cache_d * stride_k_cache_bl * stride_k_cache_x + 
            ...
            off_v = (
                bn[:, None] + layer_id * stride_v_cache_h * stride_v_cache_d * stride_v_cache_bl +
    ...
    def _block_fwd_kernel_alibi(
        ...
        for start_n in range(0, cur_batch_ctx_len, BLOCK_N):
            ...
            off_k = (bn[None, :] + layer_id * stride_k_cache_h * stride_k_cache_d * stride_k_cache_bl * stride_k_cache_x + 
                     ...
                     (offs_d[:, None] % x) * stride_k_cache_x)
            off_v = (
                bn[:, None] + layer_id * stride_v_cache_h * stride_v_cache_d * stride_k_cache_bl + 
                cur_kv_head * stride_v_cache_h +
                offs_d[None, :] * stride_v_cache_d +
                (start_n + offs_n[:, None]) % block_size * stride_v_cache_bl)
            k = tl.load(K_cache_addr + off_k,
    ...
    def context_attention_block_fwd(layer_id,
                                    block_size,
                                    num_kv_heads,
                                    head_size,
                                    x,
        ...
        if alibi_slopes is not None:
            _block_fwd_kernel_alibi[grid](
                ...
                b_ctx_len,
                alibi_slopes,
                block_size, #v_cache.shape[3], # [num_blocks, num_heads, head_size, block_size]
                8,
                o,
                b_loc.stride(0),
                b_loc.stride(1),
