...
class LLMEngine:
    ...
    def _init_transfer_workers(self):
        ...
        print("len gpu_addr ", len(gpu_addr))
        print("len gpu_addr ", len(self.model_executor.workers))
        for i in range(len(gpu_addr)):
            if i == 0:
                ...
                self.transfer_workers.append(TransferWorker(gpu_addr[i], self.cache_config, self.model_config, self.parallel_config, self.deploy_config, self.model_executor.workers[i].rank, self.model_executor.workers[i].device_id))
    ...
    def add_kv_response(
        ...
        # print("fetch_kv_blocks blocks ", response.computed_blocks, len(blocks[response.computed_blocks:]))
            print("test for vllm p kv response ", request_id, time.time())
    ...
    def add_request(
        ...
        if not self.deploy_config.enable_separate or self.deploy_config.role == 'prompt':
            ...
            print("test for vllm p add request ", seq_group.request_id, time.time())
        else:
            self.scheduler.add_decode_seq_group((seq_group, prefill_request_output))
            print("test for vllm d add request ", seq_group.request_id, time.time())
        return kv_response
