...
class LLMEngine:
    ...
    def __init__(
        ...
        self.send_kv_trans_scheduler = SendKvTransferScheduler(self.parallel_config.tensor_parallel_size, self.deploy_config.enable_layer, self.deploy_config.role, self.deploy_config.use_agg_block)
        
        self.recv_kv_trans_scheduler = RecvKvTransScheduler(self.parallel_config.tensor_parallel_size, self.deploy_config.enable_layer, self.deploy_config.role, self.deploy_config.use_agg_block)
    ...
    def add_kv_results_request(
        ...
        if can_allocate == AllocStatus.OK or not self.deploy_config.enable_trans_to_dram:
            if can_allocate == AllocStatus.OK:
                ...
                if self.deploy_config.enable_theory:
                    ...
                    if blocks:
                        ...
                        print("request_output.prompt request id ", request_id, seq.data.prompt_token_ids)
            ...
            if can_allocate == AllocStatus.OK:
                ...
                kv_response = KvPreparedResponse(seq_group.request_id, 0, None, 0, -1, None , True)
        return kv_response
