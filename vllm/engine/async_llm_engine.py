...
class _AsyncLLMEngine(LLMEngine):
    ...
    async def _notify(self, request_id, seq_group: SequenceGroup):
        if self.scheduler.deploy_config.role == "prompt":
            decode_entry_point = (seq_group.edecode_host, seq_group.edecode_port)
        else:
            decode_entry_point = (seq_group.eprefill_host, seq_group.eprefill_port)
        data = CommData(
            headers=CommonHeader(self.deploy_config.deploy_host, self.deploy_config.deploy_port).__json__(),
    ...
    async def step_async(self, request_tracker) -> List[RequestOutput]:
        ...
        if self.scheduler.cache_config.enable_radix_caching and self.scheduler.cache_config.enable_radix_evictor:    
            ...
            if evict_hbm_nums:
                evicted_nums = self.scheduler.evict_radix_tree(evict_nums=evict_hbm_nums, device=Device.GPU)
                print("evict_hbm_nums ", evicted_nums)
            # if is_hbm_evict:
