...
#record infight req id & decode instance, to use in d->p and at least load
infight_prefill_req: Dict[InstanceInfo, int] = {}
infight_decode_req: Dict[InstanceInfo, int] = {}
...
async def monitor_report(request: Request) -> Response:
    ...
    if engine_type == EngineType.EPREFILL.value:
        if infight_prefill_req.get(key):
            ...
            infight_prefill_req[instance] = 0
    else:
        if infight_decode_req.get(key):
            instance = infight_decode_req[key]
            instance.num_unfinished_reqs = num_unfinished_requests
            # instance['unfinished_tokens'] = unfinished_tokens
            instance.used_gpu_blocks = used_gpu_blocks
            instance.used_cpu_blocks = used_cpu_blocks
