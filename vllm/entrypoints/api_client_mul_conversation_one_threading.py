...
def sample_requests(
    ...
    for data in dataset:
        ...
        while index < count:
            ...
            session_info.append((input_value_token_ids[1:4], len(output_value_token_ids[1:4])))
            session_len =  session_len + len(input_value_token_ids[1:4]) +  len(output_value_token_ids[1:4])  
            index = index + 2
        if session_len < 4096:
