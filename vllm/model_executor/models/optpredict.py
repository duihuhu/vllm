...
class OPTChunkedForSequenceClassification(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.score = nn.Linear(config.word_embed_proj_dim, 10, bias=False)
    ...
    def forward(
        ...
        cache_events: Optional[List[torch.cuda.Event]],
        chunkinputmetadata: ChunkInputMetadata
    ) -> List[int]: #Tuple[List[int], List[float]]: #torch.Tensor: #Tuple[Dict[int, SequenceOutputs], torch.Tensor]:
        #st1 = time.time()
        hidden_states = self.model(input_ids, positions, kv_caches, #input_metadata, 
                                   cache_events, chunkinputmetadata)
        hidden_states = hidden_states[chunkinputmetadata.idxs]
        #ed1 = time.time()
        #print(f"In opt.py hidden_states is {hidden_states.shape}")
        #st2 = time.time()
        logits = self.score(hidden_states)
        predict_label = torch.argmax(logits, dim=1).tolist()
