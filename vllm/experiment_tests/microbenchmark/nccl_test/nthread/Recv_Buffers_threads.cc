...
void nccl_recv(int thread_id, long long BUF_SIZE, int NUM_BUFFERS, ncclComm_t comm, int device_id);
...
void nccl_init(int thread_id, long long BUF_SIZE, int NUM_BUFFERS, int numGPUs, ncclComm_t comm, ncclUniqueId commId, int device_id){
    ...
    // std::cout<<"nccl_recv  "<<std::endl;
    nccl_recv(thread_id, BUF_SIZE, NUM_BUFFERS, comm, device_id);
}
...
void nccl_recv(int thread_id, long long BUF_SIZE, int NUM_BUFFERS, ncclComm_t comm, int device_id) {
    ...
    // Warm up: receive buffers
    long long buffer_size = BUF_SIZE;
    ncclGroupStart();
    for (int i = 0; i < NUM_BUFFERS; ++i) {
        NCCL_CHECK(ncclRecv(recv_buf[i], buffer_size, ncclFloat, 0, comm, streams));
    }
    ncclGroupEnd();
    cudaStreamSynchronize(streams);

    // Benchmark: receive buffers and measure time
    ...
    std::cout << "Thread " << thread_id << " Receive Copying time for buffer: "
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < NUM_STREAM; ++i) {
        ...
        close(fd);
        threads.emplace_back(nccl_init, i, BUF_SIZE, NUM_BUFFERS, numGPUs, comm[i], commId[i], 1); // device_id set to 0
        while(1){
            if(running[i]==1){
        ...
        std::cout<< "while out not " << i << " " << ready_num<< std::endl;
    ...
    while(1){
        if(ready_num == NUM_STREAM){
            ...
            std::cout<< "ready_num not " << ready_num;
        }
    }

    // Create threads and start NCCL receive operations
    // for (int i = 0; i < NUM_STREAM; ++i) {
    //     threads[i] = std::thread(nccl_recv, i, BUF_SIZE, NUM_BUFFERS, comm[i], 1);
    // }
