...
def notify_mdecode():
    ...
    while True:
        if prefill_nums != mm[(already_num*35+34):(already_num*35+35)]:
            ...
            engine.convert_req_label_status_dict(request_id, label, arrive_time)
...
def init_mdecode_prefill():
    ...
    while True:
        ...
        if mdecode_status == "init_mdecode_prefill":
            decode_event.wait()
            p_iteration_num = engine.generate_mdecode_prefill(p_iteration_num)
...
async def init_mdecode(request: Request) -> Response:
    ...
    reused_request_ids = request_dict.pop("reused_request_ids")
    ...
    engine.add_reused_request_ids(reused_request_ids)
    engine.total_num_requests = len(prompts)
    decode_event.set()
