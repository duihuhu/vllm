...
def post_init_decode_prefill(prompts: List[str],
                      ...
                      n: int = 1,
                      stream: bool = False,
                      repreated_request_ids: List[str] = []) -> requests.Response: 
    ...
    pload = {
        ...
        'ignore_eos': True,
        "stream": stream,
        "repreated_request_ids":repreated_request_ids,
    }
...
if __name__ == "__main__":
    ...
    for rep in range(repreated):
        for prompt in range(len(sampled_prompts)):
            reused_request_ids.append(random_uuid())
    ...
    task_td = []
    task_td.append(threading.Thread(target=post_init_decode_prefill, args=(prompts, output_lens, request_ids, api_url_decode , n, stream, reused_request_ids)))
