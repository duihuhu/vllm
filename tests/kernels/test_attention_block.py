...
def run_new_single_query_cached_kv_attention() -> None:
    ...
    query = torch.empty(3, 40, 128, dtype = torch.float16).to(device)
    ...
    block_tables_tensor = torch.tensor(block_tables_list, dtype = torch.int).to(device)

    context_lens_list = [32, 16, 48]
    context_lens_tensor = torch.tensor(context_lens_list, dtype = torch.int).to(device)
    ...
    output = torch.empty(3, 40, 128, dtype = torch.float16).to(device)
    ...
    output2 = torch.empty(3, 40, 128, dtype = torch.float16).to(device)
    ...
    block_tables_tensor2 = torch.tensor(block_tables_list2, dtype = torch.int).to(device)

    context_lens_list2 = [32, 16, 48]
    context_lens_tensor2 = torch.tensor(context_lens_list2, dtype = torch.int).to(device)
    ...
    query2 = torch.empty(1, 40, 128, dtype = torch.float16).to(device)
    ...
    block_tables_tensor3 = torch.tensor(block_tables_list3, dtype = torch.int).to(device)
    context_lens_list3 = [4096]
    context_lens_tensor3 = torch.tensor(context_lens_list3, dtype = torch.int).to(device)
    max_context_len2 = 4096
    output3 = torch.empty(1, 40, 128, dtype = torch.float16).to(device)
    ...
    output4 = torch.empty(1, 40, 128, dtype = torch.float16).to(device)
    ...
    block_tables_tensor4 = torch.tensor(block_tables_list4, dtype = torch.int).to(device)
    context_lens_list4 = [4096]
    context_lens_tensor4 = torch.tensor(context_lens_list4, dtype = torch.int).to(device)
