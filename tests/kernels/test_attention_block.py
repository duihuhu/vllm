...
def run_new_single_query_cached_kv_attention(v) -> None:
    ...
    if v == 1:
        ...
        exp_sums2 = torch.empty(
                size = (3, 40, 512),
                dtype = torch.float32)
        max_logits2 = torch.empty_like(exp_sums1)
        ...
        key_cache3 = torch.empty(size = (32, 10, 128 // x, 16, x),
                                dtype = torch.float16,
                                device = 'cuda')
        ...
        value_cache3 = torch.empty(size = (32, 10, 128, 16),
                                dtype = torch.float16,
                                device = 'cuda')
