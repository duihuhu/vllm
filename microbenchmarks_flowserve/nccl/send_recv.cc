...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < NUM_COMM; ++i) {
        ...
        close(fd);
        threads.emplace_back(nccl_init, i, BUF_SIZE, NUM_BUFFERS / NUM_COMM, NUM_STREAMS, numGPUs, comm[i], commId[i], device_id); // device_id set to 0
        while(1){
            if(running[i] == 1){
